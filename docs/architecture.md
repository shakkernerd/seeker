...
A restart turns interrupted sends into `unknown`; it does not blindly repeat them. Definite retryable failures respect adapter delay hints and stop after five attempts. An authenticated reconnect or configured adapter startup can resume known-unaccepted exhausted work. The durable reply remains available to its manager even when a push attempt is rejected or uncertain. Terminal channel failures produce one operational notice for that manager, with no recursive notice on host failure.

At most four outbound operations are active, with one per channel or manager ownership generation. An operation that ignores cancellation is quarantined with that one unsettled call. Other managers and verified successors can continue. Total outstanding I/O, including quarantined calls, is capped at 32; reaching that fault limit pauses new sends until calls settle or the service is restarted. Attempt owner generations fence late results after a transfer.
