...
At most four outbound operations are active, with one per channel or manager binding. An operation that ignores cancellation is quarantined with that one unsettled call. Other managers on the same host can continue, and replacement I/O cannot accumulate. Attempt owner generations fence late results after a transfer.
