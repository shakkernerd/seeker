...
function fixture() {
  ...
  const opened: DesktopProfile[] = [];
  const operations = {
    inspect: async () => instances,
    start: async (value: DesktopProfile, signal: AbortSignal) => { signal.throwIfAborted(); opened.push(value); instances = [observed(owner)]; },
    pipe: () => "/private/qualified-app-tools.sock",
  };
...
test("service restart can reuse the pinned live owner or start its stopped profile without selecting a task", async () => {
  const f = fixture();
  ...
  await restarted.prepare(new AbortController().signal, () => true);
  expect(f.opened).toHaveLength(0);
  f.instances([]);
  await restarted.prepare(new AbortController().signal, () => true);
  expect(f.opened).toEqual([f.profile]);
});

test("a manually restarted app can qualify the helper only with the same actual native profile", async () => {
  const f = fixture();
  await f.lifecycle.connected(f.owner); await f.lifecycle.admitted(f.owner);
  ...
  await f.lifecycle.prepare(new AbortController().signal, () => true);
  expect(f.opened).toHaveLength(0);
  ...
  await expect(f.lifecycle.prepare(new AbortController().signal, () => true)).rejects.toThrow("registered native profile and storage");
  expect(f.opened).toHaveLength(0);
  f.instances([observed(replacement)]);
...
test("multiple running instances and changed native storage cannot replace registration", async () => {
  ...
  await expect(f.lifecycle.prepare(new AbortController().signal, () => true)).rejects.toThrow("More than one Desktop");
...
test("cancellation after owner inspection prevents the launch command", async () => {
  ...
  await expect(cancelled.prepare(controller.signal, () => true)).rejects.toThrow();
  expect(f.opened).toHaveLength(0);
});

test("a manager transfer during asynchronous inspection prevents background startup", async () => {
  const f = fixture();
  await f.lifecycle.connected(f.owner); await f.lifecycle.admitted(f.owner);
  let current = true;
  const transferred = new CodexDesktopLifecycle(f.statePath, { ...f.operations, inspect: async () => { current = false; return []; } });
  await expect(transferred.prepare(new AbortController().signal, () => current)).rejects.toThrow("manager assignment changed");
  expect(f.opened).toHaveLength(0);
});
...
test("missing native storage evidence cannot fall back to a cold-start default", async () => {
  ...
  f.instances([]);
  await expect(f.lifecycle.prepare(new AbortController().signal, () => true)).rejects.toThrow("original native storage directory");
  expect(f.opened).toHaveLength(0);
});
