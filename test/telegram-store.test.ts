...
test("late stop on an uncertain send remains correlated after its answer was handled", async () => {
  ...
  expect(receipt.source.reference).toBe(String(original.message_id));
  expect(receipt.source.eventId).toStartWith("callback:");
  ...
  f.restart();
  expect(f.core.channel(f.channel.id).resolveMessage("42", String(original.message_id))).toBe(f.store.get("lost-send")!.exchange.revisions[0]!.replyHandle);
  const directReference = { ...original };
  delete directReference.reply_markup;
  f.updates.push(reply(2, "/stop Do not publish anything else", directReference));
  await f.poll(); await f.flush();
...
test("inaccessible callback messages retain authenticated handles while stale revisions remain rejected", async () => {
  ...
  f.advance(); await f.flush();
  const current = f.sent.find((message) => message.text.includes("Publish to preview-two?"))!;
  const wrongReference = click(4, "known-reference-mismatch", current);
  wrongReference.callback_query.data = original.reply_markup.inline_keyboard[0][0].callback_data;
  f.updates.push(wrongReference);
  await f.poll();
  expect(f.calls.filter((call) => call.method === "answerCallbackQuery").at(-1)!.body.text).toBe("This action does not match the request.");
  expect(f.core.channel(f.channel.id).resolveMessage("42", String(current.message_id))).toBe(manager.get("inaccessible-request").exchange.revisions[1]!.replyHandle);
  f.updates.push(inaccessible(5));
  await f.poll();
